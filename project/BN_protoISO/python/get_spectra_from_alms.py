...
for task in subtasks:
    ...
    # If this is true, skips all auto/cross computations to prevent errors
    no_cross: bool = (np.array(ps_dict[spec, "cross"]).shape[0] == 0.)
    no_auto: bool =  (np.array(ps_dict[spec, "auto"]).shape[0] == 0.)
    ...
    for spec in spectra:
        ...
        if spec == 'TT':
            print(spec_name_cross, np.array(ps_dict[spec, "cross"]).shape, np.array(ps_dict[spec, "auto"]).shape)

        if ar1 == ar2 and sv1 == sv2:
            # Average TE / ET so that for same array same season TE = ET
            if (np.array(ps_dict[spec, "cross"]).ndim == 1) and (np.array(ps_dict[spec[::-1], "cross"]).ndim == 1):
                ps_dict_cross_mean[spec] = (np.array(ps_dict[spec, "cross"]) + np.array(ps_dict[spec[::-1], "cross"])) / 2.
            else:
        ...
        if sv1 == sv2:
            ...
            if (not no_cross) and (not no_auto):
